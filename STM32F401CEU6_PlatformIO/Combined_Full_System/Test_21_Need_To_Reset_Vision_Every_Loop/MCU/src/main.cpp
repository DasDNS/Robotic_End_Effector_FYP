...
#define SERVO_MIN_US 500   // Min = 500 
#define SERVO_MAX_US 2400

#define FULL_SWEEP_TIME_SEC 8.0f
#define RESET_SWEEP_TIME_SEC 6.0f    // ungrasp (500 -> 2400)

#define CURRENT_PRINT_PERIOD_MS 200
#define FSR_PRINT_PERIOD_MS 200

// Targets (requested)
#define FAST_TARGET_US      1600
...
float resetRampSpeedUsPerSec = fullTravelUs / RESET_SWEEP_TIME_SEC;
...
void startResettingNow() {
  ...
  startRampAllToUsingGlobalMul(SERVO_MAX_US, resetRampSpeedUsPerSec, false);

  Serial.println("[UI] RESET: Opening/spreading at RESET speed...");
}
...
void loop() {
  ...
  if (nowMs - lastControlMs >= CONTROL_PERIOD_MS) {
    ...
    switch (state) {
      ...
      case STATE_RECOVER: {
        // In SETTLE we are already at SERVO_MIN_US (500).
        ...
        if (!recoverTo1200Commanded) {
          ...
          Serial.println("[FSM] RECOVER: spread -> 900us");
          break;
        }
