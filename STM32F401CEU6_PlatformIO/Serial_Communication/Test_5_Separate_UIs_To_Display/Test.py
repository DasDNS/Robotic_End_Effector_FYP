...
# ----------------------------
# Small helper: keep UI fast (avoid infinite text growth)
# ----------------------------
def append_limited(text_edit: QTextEdit, line: str, max_lines: int = 400):
    """
    Append a line to QTextEdit but keep only last max_lines.
    This prevents UI slowing down / freezing when lots of serial messages arrive.
    """
    # Quick path if small
...
class SerialManager(QObject):
    ...
    # routed outputs
    sig_status_line = Signal(str)   # LED ON/OFF/BLINK etc + everything else
    sig_fsr_line = Signal(str)      # FSR Readings lines
    sig_current_line = Signal(str)  # Current readings lines

    sig_log_line = Signal(str)      # debug log (TX/RX/conn)
    ...
    def _reader_loop(self):
        while not self._stop:
            try:
                ...
                # -------- ROUTING / FILTERING --------
                # You can change these rules easily later:
                # - FSR lines: start with "FSR"
                # - Current lines: start with "Current"
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.btn_clear = QPushButton("Clear Monitors")  # NEW
        ...
        quick_layout.addWidget(self.btn_clear)
        ...
        # NEW: clear monitors
        self.btn_clear.clicked.connect(self.clear_monitors)
        ...
        # Route into separate UI components (limited lines)
        ...
        self.serial_mgr.sig_log_line.connect(lambda s: append_limited(self.log_box, s, max_lines=400))
    ...
    def refresh_ports(self):
        ...
        append_limited(self.log_box, "Ports refreshed.", max_lines=400)
    ...
    def auto_detect_port(self):
        ...
        append_limited(self.log_box, f"Auto-detected port: {port}", max_lines=400)
    ...
    def on_error(self, msg: str):
        append_limited(self.log_box, f"ERROR: {msg}", max_lines=400)
