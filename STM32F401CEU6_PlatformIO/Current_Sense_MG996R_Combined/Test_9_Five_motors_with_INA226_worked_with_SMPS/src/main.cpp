...
// One INA object per channel (same style as your working code)
...
// ===============================
// INA presence check at 0x40 (on CURRENT selected PCA channel)
// ===============================
...
// ===============================
// I2C ERROR CHECK (same style as ref)
// ===============================
...
// Format: millis,pulseWidth,S0=..,S1=..,S2=..,S3=..,S4=..
// ===============================
...
// ===============================
// MOVE ALL SERVOS (same behavior as your old code)
// ===============================
...
void setup() {
  ...
  Serial.println("\n=== STM32 Black Pill: 5 Servo + 5 INA226 (PCA9548A ch0..4) ===");
  ...
  // -------- INIT INA226 SENSORS WITH CHECKS (presence + init) --------
  selectPCAChannel(0);
  if (!inaPresentOnCurrentBus()) {
    Serial.println("INA226 NOT FOUND on PCA channel 0. Halting.");
    while (1) {}
  }
  if (!ina226_0.init()) {
    Serial.println("INA226 init FAILED on PCA channel 0. Halting.");
    while (1) {}
  }

  selectPCAChannel(1);
  if (!inaPresentOnCurrentBus()) {
    Serial.println("INA226 NOT FOUND on PCA channel 1. Halting.");
    while (1) {}
  }
  if (!ina226_1.init()) {
    Serial.println("INA226 init FAILED on PCA channel 1. Halting.");
    while (1) {}
  }

  selectPCAChannel(2);
  if (!inaPresentOnCurrentBus()) {
    Serial.println("INA226 NOT FOUND on PCA channel 2. Halting.");
    while (1) {}
  }
  if (!ina226_2.init()) {
    Serial.println("INA226 init FAILED on PCA channel 2. Halting.");
    while (1) {}
  }

  selectPCAChannel(3);
  if (!inaPresentOnCurrentBus()) {
    Serial.println("INA226 NOT FOUND on PCA channel 3. Halting.");
    while (1) {}
  }
  if (!ina226_3.init()) {
    Serial.println("INA226 init FAILED on PCA channel 3. Halting.");
    while (1) {}
  }

  selectPCAChannel(4);
  if (!inaPresentOnCurrentBus()) {
    Serial.println("INA226 NOT FOUND on PCA channel 4. Halting.");
    while (1) {}
  }
  if (!ina226_4.init()) {
    Serial.println("INA226 init FAILED on PCA channel 4. Halting.");
    while (1) {}
  }
